						...
						<v-card-text>
							<v-row justify="center">
								<v-col cols="12" sm="6" md="4">
									<v-text-field color="#9090ee" type="number" :rules="[rules.minLength]" v-model="editedItem.amount" label="kwota"></v-text-field>
								</v-col>
								<v-col cols="12" sm="6" md="4">
	...
	methods: {
		save(category) {
			...
			if (this.editedIndex > -1) {
				...
				const item = this.$route.path === "/expenses" ? { expenseId: this.editedItem.id, amount: parseInt(this.editedItem.amount) } : { incomeId: this.editedItem.id, amount: parseInt(this.editedItem.amount) };
				const url =
		...
		deleteItem(item) {
			...
			if (confirm("Are you sure you want to delete this item?")) {
				...
				? "/api/Expenses/Delete"
				: "/api/Income/Delete";
				const tableItem = this.$route.path === "/expenses" ? { expenseId: item.id } : { incomeId: item.id };
				axios.delete(url, { data: tableItem, headers: { 'Content-Type': 'application/json' } }).then(() => {
					this.saveToDatabase();
				});
			}
		},
